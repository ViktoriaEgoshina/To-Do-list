class Todo {
  ...
  getItemsFromLocalStorage() {
    ...
    try {
      ...
      return Array.isArray(parsedData) ? parsedData : [] /*Проверяем: соответствуют ли данные*/
    } catch {
      console.error('Error getting items from local storage')
      return [] /*В случае ошибки - не останавливаем приложение*/
    }
  }
...
<li
    ...
    const isEmptyFilteredItems = this.state.filteredItems?.length === 0 /*Пустой ли список отфильтрованных элементов*/
    const isEmptyItems = this.state.items.length === 0

    this.emptyMessageElement.textContent =
      isEmptyFilteredItems ? 'Задачи не найдены'
      : isEmptyItems ? "Задач нет"
        : ''
  }
